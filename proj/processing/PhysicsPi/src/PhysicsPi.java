import processing.core.*; //~Java
import java.lang.*; //~Java
import java.text.DecimalFormat; //~Java
...
public class PhysicsPi extends PApplet{
  class Bouncer {
    ...
    public void show(){
      ...
      DecimalFormat massFormat = new DecimalFormat("0E00"); //~Java
      String massStr = massFormat.format(this.mass) + "g"; //~Java
      //~Script float massStr = (float)this.mass
  ...
  public void setup(){
    //~Script settings();
    initBlocks(digits);
    surface.setResizable(true); //~Java
  }
  public void initBlocks(int digits){
    println("Digits of Pi: " + digits); //~Java
    eulerStep = (int)pow(10,digits-1);
    if(digits == 1){ eulerStep = 1; }
    println("Calculations per Frame:" + eulerStep); //~Java
  ...
  public void keyPressed(){
    //println(keyCode);
    //~Script if(49 <= keyCode && keyCode <= 56){
    if(49 <= keyCode && keyCode <= 57){ //~Java
      piCounter = 0;
      doCount = true;
      digits = keyCode-48;
      initBlocks(digits);
