...
function showStatus(title, content) {
  ...
  $("#statusMessage").children()[0].innerHTML = title;
  $("#statusMessage").children()[1].innerHTML = content;
  $("#statusMessage").show();
...
function onWsError(e){
  isConnected = false;
  showStatus("Error", "Couldn't connect to the server " + ws.url);
}
function onWsOpen() {
  var msg;
  delay = 500;
  ...
  wsSend(msg);
  sendNickName();
  log.info("Connection successful!", ws.url);
  isConnected = true;
  hideStatus();
}
function onWsClose() {
  isConnected = false;
  showStatus("Websocket Closed", "No Idea Why");
}
...
function drawGameScene() {
  ...
  if (chatCanvas != null){
    if(chatCanvas.width != 0 && chatCanvas.height != 0){
...
var CONNECTION_URL = location.hostname + ":11011";
ws = null
isConnected = false;
var leftTouchID = -1,
...
$(document).ready(function(){
  hideStatus();
  $("#statusMessage").hover(function() {
    console.log("Hovering");
    $('#statusMessage').stop(true).fadeTo(0, 1);
  }, function() {
    $('#statusMessage').stop(true).fadeOut(2000);
  });
  
  $("#custumUrl").change( function() {
    console.log("Setting custum url");
    setserver($("#custumUrl")[0].value, true);
  });
  $("#gamemode").on("change", function(){
    var selectedGamemode = $(this).children("option:selected")
    if(selectedGamemode.text() == "Custom"){
      $("#custumUrl").show();
      CONNECTION_URL = null;
      ws.close();
    }else{
      setserver(selectedGamemode.val());
    }
  });
  $("#gamemode").children(1)[0].value = CONNECTION_URL;
  ...
  window.setSkins = function(arg) {
    showSkin = arg
  };
  window.setNames = function(arg) {
    showName = arg
  };
  window.setDarkTheme = function(arg) {
    showDarkTheme = arg
  };
  window.setColors = function(arg) {
    showColor = arg
  };
  window.setShowMass = function(arg) {
    showMass = arg
  };
  window.setSmooth = function(arg) {
    smoothRender = arg ? 2 : .4
  };
  window.setChatHide = function(arg) {
    hideChat = arg;
    if (hideChat) {
      $('#chat_textbox').hide();
    } else {
  ...
  window.setAcid = function(arg) {
    xa = arg
  };
  window.openSkinsList = function(arg) {
  ...
  if (window.localStorage != null) {
    $(".save").each(function() {
      ...
      var value = window.localStorage.getItem("checkbox-" + id);
    ...
    $(".save").change(function() {
      ...
      window.localStorage.setItem("checkbox-" + id, value);
    });
    if (window.localStorage.AB8 == null) {
      window.localStorage.AB8 = ~~(100 * Math.random());
    }
  }
  ...
  console.log("Loaded ready to play on: "+CONNECTION_URL);
  gameLoop();
});
